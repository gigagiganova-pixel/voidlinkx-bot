...
function buildStartText() {
    return [
        '🛰 <b>VOIDLINK X</b>',
        '<b>Твой закрытый канал криптоанализа и связи</b>',
        '',
        'Приватный браузерный командный центр: крипто-радар, голосовая комната, чат и персональный WebRTC/P2P-канал в одной рабочей ссылке.',
        '',
        '📊 CRYPTO-панель для наблюдения за рынком',
        '🎙 Голосовая комната и чат прямо в браузере',
        '🛡 Персональная ссылка из ограниченного пула',
        '🔒 Без серверной записи разговоров и переписок',
        '🏆 Купленный канал закрепляется за вами',
        '',
        buildPriceLine()
    ].join('\n');
}
...
function buildAboutText() {
    return [
        '🛡 <b>VOIDLINK X: приватный крипто-командный центр</b>',
        '',
        'VOIDLINK X объединяет крипто-дашборд, голосовую комнату, чат и приватное WebRTC/P2P-подключение. Всё открывается по персональной рабочей ссылке, которую бот выдаёт после ручной проверки оплаты.',
        '',
        '<b>Что внутри:</b>',
        '📊 CRYPTO: визуальная панель наблюдения за рынком.',
        '🎙 VOICE ROOM: отдельная голосовая комната.',
        '💬 CHAT: быстрый приватный диалог.',
        '👥 USERS: подключение участников по личным ссылкам.',
        '🧠 BEHAVIOR: дополнительный блок оценки поведения/сигналов.',
        '',
        '<b>Как устроен доступ:</b>',
        `${buildPriceLine().replace(/<[^>]+>/g, '')} — персональный канал из ограниченного пула.`,
        '🔗 После подтверждения вы получаете оригинальную рабочую ссылку.',
        '🛡 Ссылка удаляется из общего пула и закрепляется за вашей покупкой.',
        '',
        'Это не публичная страница “для всех”, а отдельный экземпляр браузерной системы, который выдаётся персонально.'
    ].join('\n');
}
...
async function configureBotProfile() {
    const description = [
        'Закрытый канал криптоанализа и приватной связи.',
        'VOIDLINK X — браузерный командный центр с CRYPTO-панелью, голосовой комнатой, чатом и персональным WebRTC/P2P-доступом.',
        'После подтверждения оплаты вы получаете оригинальную рабочую ссылку из ограниченного пула.'
    ].join('\n');

    try {
        await bot.setMyShortDescription({ short_description: 'Закрытый канал криптоанализа, связи и персонального WebRTC/P2P-доступа.' });
